...
def get_history_list(user: PolarUser,
    ...
    print(f'Begin to get the history, from {start_date} to {end_date}')
    try:
...
def sync(email, password, save_dir, start_date, end_date):
    ...
    histories = get_history_list(user, start_date, end_date)
...
@click.option('-s',
              '--start-date',
              default='2021-02-14',
              help='Date string, like "2021-02-14"')
@click.option('-e',
              '--end-date',
              default=str(date.today()),
              help='Default is today')
def main(email, password, config_file, save_dir, start_date, end_date):
    save_dir = Path(save_dir)

    if email is not None and password is not None:
        sync(email, password, save_dir, start_date, end_date)
    if config_file is not None:
        ...
        for i, row in df.iterrows():
            ...
            print(f'\nSyncing [{i + 1}/{total}] - For account: {email}')
            sync(email, password, save_dir, start_date, end_date)
