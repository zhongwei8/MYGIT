...
LastEditTime : 2021-04-07 12:40:45
FilePath     : /activity-recognition/src/py/activity_recognizer_demo2.py
'''
...
def plot_result(standard_accel_df: pd.DataFrame,
    ...
    plt.plot(standard_accel_df['EventTimestamp(ns)'].values[1:],
             np.diff(standard_accel_df['EventTimestamp(ns)'].values),
             '-o',
             markersize=2,
             linewidth=1,
    ...
    if 'STD' in result_df.columns:
        ...
        axes[0].plot(result_df['EventTimestamp(ns)'],
                     std,
                     '-o',
                     markersize=2,
                     linewidth=1,
    ...
    axes[1].set_yticklabels(HAR_TYPE_NAMES)
...
def get_standard_accel(data_path: Path, is_record: bool) -> pd.DataFrame:
    if data_path.is_dir():
        ...
        if data_path.suffix == '.csv':
            standard_accel_df = get_standard_accel_from_raw_csv(data_path)
            return standard_accel_df
...
def run(data_path: Path, is_record: bool):
    standard_accel_df = get_standard_accel(data_path, is_record)

    result = process_standard_accel_xyz_26hz(standard_accel_df)

    plot_result(standard_accel_df, result, data_path.name)
...
@click.option('-r',
              '--is-record',
              is_flag=True,
              help='Data path is a record dir')
